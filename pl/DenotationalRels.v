...
Proof.
  ...
  - destruct H as [x1 [[H1|H1] H2]].
    ...
    + right.
      exists x1.
      tauto.
  - destruct H as [[x1 [H1 H2]]|[x1 [H1 H2]]].
    + exists x1.
      tauto.
    + exists x1.
      tauto.
...
Corollary e1: forall e c1 c2,
[[ if (e) then { c1 } else { skip }; c2]] ~=~
[[ if (e) then { c1; c2 } else { c2 } ]].
Proof.
  intros.
  rewrite if_seq.
  rewrite skip_seq.
...
Lemma test_comm: forall (X Y: state -> Prop), Rels.test X ∘ Rels.test Y == Rels.test Y ∘ Rels.test X.
Proof.
  intros.
  apply Sets_equiv_Sets_included; split.
  + Sets_unfold; intros.
