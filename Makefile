...
ASSIGNMENT_FLAG = -R $(ASSIGNMENT_DIR) Assignment -R $(PL_DIR) PL -R $(SETS_DIR) SetsClass -R $(COMPCERT_DIR) compcert.lib

DEP_FLAG = -R $(ASSIGNMENT_DIR) Assignment -R $(PL_DIR) PL -R $(SETS_DIR) SetsClass -R $(COMPCERT_DIR) compcert.lib
...
PL_FILE_NAMES = \
    Syntax.v SimpleProofsAndDefs.v InductiveType.v DenotationalBasic.v RecurProp.v BuiltInNat.v DenotationalRels.v Logic.v FixedPoint.v Monad.v List.v Monad2.v

PL_FILES=$(PL_FILE_NAMES:%.v=$(PL_DIR)/%.v)

ASSIGNMENT_FILE_NAMES = \
	Test.v Test1.v DenotationalWithProcedureCall.v
